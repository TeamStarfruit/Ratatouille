...
public class CompostData {
    ...
    private boolean canOutput(ItemStackHandler outputInventory, IFluidHandler fluidHandler, RandomSource randomSource) {
        for (ItemStack outputStack : lastRecipe.rollResults(randomSource)) {
    ...
    public boolean evaluate(CompostTowerBlockEntity tower) {
        if (tower.getLevel() == null) return false;
    ...
    public @NotNull MutableComponent getLevelComponent() {
        ...
        if (compostTowerLevel == 0) {
            ...
            return compostTowerLevel >= 8
                    ? Lang.translateDirect("compost_tower.max_lvl", new Object[0])
                    : Lang.translateDirect("compost_tower.lvl", new Object[0]).append(String.valueOf(compostTowerLevel));
        }
    }
    ...
    private void updateLastRecipe(CompostTowerBlockEntity controller) {
        ...
        try {
            ...
            Set<AbstractVariant> availableVariants =
                    RecipeTrie.getVariants(controller.inputInventory, controller.tankInventory);
            ...
            list.clear();
            for (RecipeHolder<?> r : RecipeFinder.get(getRecipeCacheKey(), controller.getLevel(), this::matchStaticFilters)) {
                var recipe = r.value();
                if (CompostingRecipe.match(controller, recipe))
